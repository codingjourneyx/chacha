    ...
    <div className="bg-gradient-to-br from-white to-purple-50 p-5 rounded-xl shadow-lg border border-purple-100 transform hover:scale-105 transition-transform duration-300">
      ...
      <div className="space-y-2">
        {businessHours.map((schedule, index) => (
          <div 
            key={index} 
            className="flex justify-between items-center py-1.5 px-2 rounded-lg transition-all duration-200 hover:bg-purple-100/50 text-sm"
          >
            <span className="font-medium text-gray-700">{schedule.day}</span>
            <span className="text-purple-700 bg-purple-100 px-3 py-1 rounded-full font-medium">{schedule.hours}</span>
          </div>
        ))}
      </div>
